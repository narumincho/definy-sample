 ...
 * @typePartId 6530304b1898543e974ad2561dc1423a
 */
...
export const SampleUser: {
  readonly codec: Codec<SampleUser>;
  /**
   * 型を合わせる上で便利なヘルパー関数
   */
  readonly helper: (a: SampleUser) => SampleUser;
} = {
  helper: (sampleUser: SampleUser): SampleUser => sampleUser,
  codec: {
    encode: (value: SampleUser): ReadonlyArray<number> =>
      String.codec
        .encode(value.name)
        .concat(Maybe.codec(Int32.codec).encode(value.ageMaybe)),
    decode: (
      index: number,
      binary: Uint8Array
    ): { readonly result: SampleUser; readonly nextIndex: number } => {
      const nameAndNextIndex: {
        readonly result: String;
        readonly nextIndex: number;
      } = String.codec.decode(index, binary);
      const ageMaybeAndNextIndex: {
        readonly result: Maybe<Int32>;
        readonly nextIndex: number;
      } = Maybe.codec(Int32.codec).decode(nameAndNextIndex.nextIndex, binary);
      return {
        result: {
          name: nameAndNextIndex.result,
          ageMaybe: ageMaybeAndNextIndex.result,
        },
        nextIndex: ageMaybeAndNextIndex.nextIndex,
      };
    },
  },
};
